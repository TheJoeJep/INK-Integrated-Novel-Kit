...
## The three questions that decide whether a scene exists

Ask these before anything else. **A scene failing all three should be cut, and saying so is the
skill's job.**

### 1. What progresses?

Plot, character understanding, or the underlying conflict. **One is enough. Zero is where readers
stop.**
...
Every obstacle must **force**, **reinforce**, or **show failure in** growth.
...
> If an obstacle does none of these, what you've added is just an event. Too many of those and it
...
**Any gorillas?**
Anything that makes a reader stop to ask an unsignalled question. Remove it, explain it in
passing, hang a lantern on it, or make it the tone promise.
...
── SCENE ── ch01/04-the-shed, beat 3
  ...
  OBSTACLE     the wound                                               ✓ shows failure
               she can't leave, so the concealment problem is forced

  PRESSING     character ✓  setting ✓  plot ✓
...
- **Say when a scene should be cut.** If it fails all three questions, say so plainly. That's the
  most valuable output this skill has and it's the one it'll be tempted to soften
...
`/dialogue` · `/beat` · `/pacing` · `/diagnose` · `/write`
